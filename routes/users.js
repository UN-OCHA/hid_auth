  ...
  async.series([
    ...
    function (cb) {
      ...
      if (options.email !== req.session.userId) {
        message = "Sorry, we do not allow changing your primary email address yet. If you need to change your email address, please contact info@humanitarian.id";
...
module.exports.resetpw = function(req, res) {
  ...
  function (err, results) {
    ...
    res.render('index', {action: 'help', options: options, message: message, redirect: req.body.redirect || '', client_id: req.body.client_id || '', redirect_uri: req.body.redirect_uri || '', temp_uri: temp_uri, csrf: req.csrfToken()});
  });
};
...
module.exports.resetpwuse = function(req, res, next) {
  ...
  if (encodedKey != undefined && String(encodedKey).length) {
    ...
    if (timestamp < (now - 7 * 86400000) || timestamp > now) {
      ...
      return next(new errors.BadRequest('<p>You have landed on this page by clicking an expired confirmation link. As a security measure, we only permit such links to be valid for a short period of time.</p><p>You can confirm your account by resetting your password.</p><p class="text-center"><a href="/#forgotPass" class="btn btn-primary">Reset Password</a></p>'));
    }
