...
function sendReminderVerifyEmails(cb) {
  ...
  stream.on('data', function(user) {
    if (user.shouldSendReminderVerify()) {
      ...
      client.get("/v0/contact/view?_access_key=" + new_access_key.toString() + "&_access_client_id=" + config.profilesClientId + "&email.address=" + user.email, function (err, req, res, data) {
        client.close();
