...
function sendReminderVerifyEmails(cb) {
  ...
  stream.on('data', function(user) {
    if (user.shouldSendReminderVerify()) {
      ...
      client.get("/v0/contact/view?_access_key=" + new_access_key.toString() + "&_access_client_id=" + process.env.PROFILES_CLIENT_ID + "&email.address=" + user.email.trim().replace(' ', ''), function (err, req, res, data) {
        client.close();
